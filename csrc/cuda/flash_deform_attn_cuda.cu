...
at::Tensor flash_deform_attn_cuda_forward(
    ...
    auto output = at::zeros({batch, num_query, num_heads, num_channels}, value.options());

    // value: (Bs, spatial_size, num_heads, num_channels) -> ptr increment per
    //          bs = spatial_size * num_heads * num_channels
    auto per_value_size = spatial_size * num_heads * num_channels;
    // offsets: (Bs, num_query, num_heads, num_levels, num_point, 4) -> ptr increment per
    //          bs = num_query * num_heads * num_levels * num_point * 4
    auto per_offset_size = num_query * num_heads * num_levels * num_point * 4;
    // output: (Bs, num_query, C=num_heads*num_channels) -> ptr increment per 
    //          bs = num_query * num_heads * num_channels
    auto per_out_size = num_query * num_heads * num_channels;

    for (int n = 0; n < batch / im2col_step_; ++n) {
        // extend switch to include Half, BFloat16, and other floating point types
        // for scalar_t
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            "flash_deform_attn_forward_cuda", ([&] {
                flash_deformable_im2col_cuda(
                    ...
                    sampling_loc_attn.data_ptr<scalar_t>() + n * im2col_step_ * per_offset_size,
                    output.data_ptr<scalar_t>() + n * im2col_step_ * per_out_size,
                    im2col_step_, spatial_size, num_heads, num_channels, num_levels,
                    num_query, num_point, d_stride, block_thread, use_reg);
            }));
    }
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            "flash_deform_attn_backward_cuda", ([&] {
                flash_deformable_col2im_cuda(
                    ...
                    im2col_step_, spatial_size, num_heads, 
                    num_channels, num_levels, num_query, num_point,
