...
class FlashDeformAttnFunction(Function):
    ...
    def forward(
        ...
        L = value_spatial_shapes.shape[0]
        expected_last = L * K * 3 + L * K
        if sampling_loc_attn.shape[-1] != expected_last:
            raise ValueError(
                "sampling_loc_attn last dimension ({}) does not match "
                "value_spatial_shapes level count L={} and K={} (expected {})".format(
                    sampling_loc_attn.shape[-1], L, K, expected_last
                )
            )
...
def ms_deform_attn_core_pytorch_3d(value, value_spatial_shapes, sampling_locations, attention_weights):
    """PyTorch reference implementation of 3D deformable attention (for debug/testing)."""
